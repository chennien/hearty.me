"use strict";$.ajaxSetup({scriptCharset:"utf-8",cache:!0});function update(data){if(window.navigator.onLine){var r=$.ajax({url:window.location.origin+"/update",type:"POST",crossDomain:!0,dataType:"json",data:data,async:!1,success:function(raw){return $(raw)["selector"]},error:function(jqXHR,textStatus,errorThrown){}});return r.status>=200&&r.status<300||r.status===304?JSON.parse(r.responseText):!1}
else{return!1}}
function is_touch_device(){if((navigator.userAgent||"").indexOf("Edge/")>0)return!1;else return"ontouchstart" in window||("maxTouchPoints" in navigator&&navigator.maxTouchPoints>0)}
...
function serverReachable(){if(window.navigator.onLine){var s=$.ajax({type:"HEAD",url:window.location.origin+"/update?"+(new Date().getTime()).toString(16),async:!1}).status;return s>=200&&s<300||s===304}
else{return!1}}
...
function timestamping(){return!Date.now?new Date().getTime():Date.now()/1e3|0}
...
function signout(f){if(f==null){alertify.set({labels:{ok:"否",cancel:"登出 🔑"},buttonReverse:!1});alertify.confirm("確定要登出嗎？",function(e){if(!e){ga_event_push("Sign Out",{event_category:"Auth",event_label:"Sign Out"});window.sessionStorage.clear();setcookie("hearty_screenlock","",1);window.location.assign("//hearty.me/signout")}})}
else{window.sessionStorage.clear();setcookie("hearty_screenlock","",1);window.location.assign("//hearty.me/signout")}}
