...
window.top.location.href=window.self.location.href;(function($){$(document).ready(function(){d1=$.when(d0,hj_getScript("//cdn.jsdelivr.net/gh/godswearhats/jquery-ui-rotatable@1.1.1/jquery.ui.rotatable.min.js"),hj_getScript("//cdnjs.cloudflare.com/ajax/libs/jqueryui-touch-punch/0.2.3/jquery.ui.touch-punch.min.js"),hj_getScript("//cdnjs.cloudflare.com/ajax/libs/interact.js/1.2.9/interact.min.js")).then(function(){stickers__initialize();editor_include_once();editor_enable()});compatibility_handle();catalog_query();editor_versioning(!0,hj_version);$(".popup-underlayer .popup, .popup.feedback, .popup.diary_screenshot a, .prologue .start, .asset .detach").on("click",function(e){e.stopPropagation()});hj_mixpanel("Post Loaded",{post_id:post_id});ga_event_push("Post Loaded",{event_category:"Posts",event_label:"Post Loaded"})})})(jQuery);var l=window.location;if(typeof history.replaceState==="function"&&l.pathname.indexOf("beta")>0)
window.history.replaceState({},document.title,l.href.replace("beta/","").replace(l.origin,""));function hj_idle(){var $t=$(".editor_toolbelt"),$sl=$(".screenlock"),toolbar_enabled=!is_touch_device()&&current_post()["editable"];$(document).mousemove(moved).keypress(moved);moved();window.setInterval(function(){var t=Number(hj__cache({bucket:"hearty_idle"}));t++;hj__cache({set:!0,bucket:"hearty_idle",data:t});switch(!0){case(t>3&&autosave_the_change()):break;case(t>6&&toolbar_enabled&&$t.is(":visible")):$t.slideUp("slow");break;case(t>119&&screenlock("status")&&$sl.is(":hidden")):screenlock(!0);break}},1000);function moved(){hj__cache({set:!0,bucket:"hearty_idle",data:0});if(toolbar_enabled)$t.slideDown("normal")}}
...
function editor_enable(on){on=on||current_post()["editable"]||!1;var $e=$(".bk-page #editor_editable"),loaded=$e.data("editor_loaded")||!1,$bn=$(".picture").find(".fa-video-plus, .fa-camera-alt");if(on){if(!loaded){editable__initialize();hj_idle();screenlock("initialize",pincode||"");hj_getScript("//hearty.me/js/jquery.uploadfile.min.js",image_uploader_initialize);$e.data("editor_loaded",!0);$bn.slideDown();$(".editor_toolbelt .kit.save").on("mouseover",autosave_the_change)}
sticker__list();mobile_editor_focus()}
else{sticker__list(!1);$bn.slideUp()}
$e.prop("contenteditable",on);toolbelt_toggle(on)}
function editor_include_once(){var post_modified=new MutationObserver(function(m){m.forEach(saved)}).observe($("#editor_editable").get(0),{characterData:!0,subtree:!0});document.addEventListener("visibilitychange",function(){if(document.hidden)autosave_the_change();else editor_versioning(!1,hj_version)});if(/iPhone|iPad|iPod/i.test(navigator.userAgent||"")){if(current_post()["editable"])window.addEventListener("pagehide",post_revise,!1)}}
...
function period__initialize(){var $pd=$(".periods");if(!$pd.data("loaded")){hj_getScript("//hearty.me/scripts/periods.min.js?v=0",function(){$pd.slideDown("slow").data("loaded",!0)})}}
...
return r.Status===1}
function post_create(){open_book(!0);if(!!getcookie("hearty_position_gps")){var gps=JSON.parse(decodeURIComponent(getcookie("hearty_position_gps")))||{}}
else if("geolocation" in navigator&&!getcookie("hearty_position_gps")){if(confirm("⛅ 是否讓我們開啟定位，並為日記加入當地天氣呢？")){$.when(window.positioning_gps_deferred).done(post_create);positioning_gps_trigger(!0);$(".mask_loading").show().delay(10000).queue(function(){$(this).fadeOut().dequeue()});msg("⛅ 麻煩您「允許」定位權限，以為您查詢天氣<br>並稍候一會兒。。。","好的");return}}
var r=gps==null?{action:"post_create"}:{action:"post_create",location_lat:gps.latitude||"",location_lon:gps.longitude||"",place:gps.address||"",weather_id:gps.weather_id||""};r=update(r);switch(r.Status){case 1:var r=r.Values,post_id=r.post_id,subject=r.title;$("<li>",{title:"📝 "+subject,onclick:"if(!post_query(this.dataset.post_id)) $(this).remove();","data-active":"","data-post_id":post_id,"data-page":1,"data-privacy":0,}).text(subject).prependTo($(".catalog .diaries"));catalog_page(1);post_query(post_id);hj_mixpanel("Post Created",{post_id:post_id});ga_event_push("Create",{event_category:"Posts",event_label:"Create"});break;case 2:signin_required();break;default:hj_alert("糟糕，出錯了，請稍候再試");break}}
