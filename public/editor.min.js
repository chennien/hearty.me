...
window.history.replaceState({},document.title,l.href.replace("beta/","").replace(l.origin,""));function hj_idle(){var $t=$(".editor_toolbelt"),$sl=$(".screenlock"),toolbar_enabled=!is_touch_device()&&current_post()["editable"];$(document).mousemove(hj_motion).keypress(hj_motion);$("#editor_editable").keydown(hj_motion);hj_motion();window.setInterval(function(){var t=Number(hj__cache({bucket:"hearty_idle"}));t++;hj__cache({set:!0,bucket:"hearty_idle",data:t});switch(!0){case(t>3&&autosave_the_change()):break;case(t>6&&toolbar_enabled&&$t.is(":visible")):$t.slideUp("slow");break;case(t>119&&screenlock("status")&&$sl.is(":hidden")):screenlock(!0);break}},1000);function hj_motion(){hj__cache({set:!0,bucket:"hearty_idle",data:0});if(toolbar_enabled)$t.slideDown("normal")}}
...
$b.scrollTop($b.find("#editor_editable").height())}}
