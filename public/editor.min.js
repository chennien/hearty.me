...
$li.filter("[data-active]").removeAttr("data-active");$li.filter("[data-post_id='"+post_id+"']").attr("data-active","").text(post.title);post.title+=" ♥ 溫度日記 Hearty Journal";document.title=post.title;$('meta[property="og:title"]').attr("content",post.title);$('meta[name="description"], meta[property="og:description"]').attr("content",post.post.replace(/\n/g,"").substring(0,48)+"…");$('meta[property="og:image"]').attr("content",post.image);$('meta[property="og:url"]').attr("content",window.location.href);$page.find(".bookmark").attr("data-bookmarked",post.bookmarked);$(".post_attr #post_created").val(post.created_date);var d=post.created_date.split("-");$page.find("header").attr("data-post_date",d[0]+"年 "+Number(d[1])+"月 "+Number(d[2])+"日");$page.find(".wi").removeAttr("class").addClass(post.weather);$page.animate({scrollTop:0},"fast");if(typeof history.replaceState==="function")
