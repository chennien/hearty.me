...
function editor_editable_scroll_to_bottom(e,el){if(is_touch_device()&&e.keyCode==13){var $pg=$(".bk-page"),range=window.getSelection().getRangeAt(0),r=document.createRange();r.selectNodeContents(el);r.setStart(range.endContainer,range.endOffset);if(r.cloneContents().textContent.toString().length<2){if(isiOS())$("body").scrollTop($("body").scrollTop()+20);else $pg.scrollTop($pg.find("#editor_editable").height())}}}
