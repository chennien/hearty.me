"use strict";if(window.top.location.hostname!==window.self.location.hostname)
...
window.history.replaceState({},document.title,l.href.replace(/beta\/|classic\//g,""));function isSafari(){return/^((?!chrome|android).)*safari/i.test(navigator.userAgent||"")}
function isiOS(){return/iPhone|iPad|iPod/i.test(navigator.userAgent||"")}
function hj_idle(){var $t=$(".editor_toolbelt"),$sl=$(".screenlock"),toolbar_enabled=!is_touch_device()&&current_post()["editable"];$(document).mousemove(hj_motion).keypress(hj_motion);$("#editor_editable").keydown(hj_motion);hj_motion();window.setInterval(function(){var t=Number(hj__cache({bucket:"hearty_idle"}));t++;hj__cache({set:!0,bucket:"hearty_idle",data:t});switch(!0){case(t>3&&autosave_the_change()):break;case(t>7&&toolbar_enabled&&$t.is(":visible")):$t.slideUp("slow");break;case(t>119&&screenlock("status")&&$sl.is(":hidden")):screenlock(!0);break}},1000);function hj_motion(){hj__cache({set:!0,bucket:"hearty_idle",data:0});if(toolbar_enabled)$t.slideDown("normal")}}
...
function load_datepicker(){$("<link>").attr({rel:"stylesheet",href:"//cdn.jsdelivr.net/gh/chennien/hearty.me@ae8b4f388ef3fed4a0c763eccbe14284872e350d/public/jquery.ui.datepicker.custom.min.css"}).add($("<link>").attr({rel:"stylesheet",href:"//hearty.me/styles/datepicker/jquery-ui.datepicker.custom.css"})).appendTo("head");hj_getScript("//cdn.jsdelivr.net/gh/chennien/hearty.me@108ecd58ef914017887125255771a919805eb5f1/public/safari.datepicker.tw.min.js",function(){$(".post_attr #post_created").attr({type:"text",onclick:"$(this).datepicker('show')"}).datepicker({dateFormat:"yy-mm-dd",showAnim:"slideDown",minDate:$(this).attr("min")||"",maxDate:$(this).attr("max")||""})})}
function editor_include_once(){var post_modified=new MutationObserver(function(m){m.forEach(saved)}).observe($("#editor_editable").get(0),{characterData:!0,subtree:!0});autosaver_30s();document.addEventListener("visibilitychange",function(){if(document.hidden)autosave_the_change();else editor_versioning(!1,hj_version)});if(isiOS()){if(current_post()["editable"])window.addEventListener("pagehide",post_revise,!1)}
if(isSafari())load_datepicker()}
function editor_editable_scroll_to_bottom(e,el){if(is_touch_device()&&e.keyCode==13){var $pg=$(".bk-page"),range=window.getSelection().getRangeAt(0),r=document.createRange(),editable_height=Number($pg.find("#editor_editable").height());r.selectNodeContents(el);r.setStart(range.endContainer,range.endOffset);if(r.cloneContents().textContent.toString().length<2){if(isiOS())$("body").scrollTop(editable_height-(window.screen.availHeight>=812?333:258));else $pg.scrollTop(editable_height)}}}
...
function period__initialize(){var $pd=$(".periods");if(!$pd.data("loaded")){hj_getScript("//cdn.jsdelivr.net/gh/chennien/hearty.me@5f4474bf50856232b418722a1a01c74dc04f6166/public/periods.min.js",function(){$pd.slideDown("slow").data("loaded",!0)})}}
...
for(var post_id in catalog){post=catalog[post_id];diaries.unshift($("<li>",{title:"📝 "+post.title,onclick:"if(this.dataset.active==null && !post_query(this.dataset.post_id)) $(this).remove();","data-post_id":post_id,"data-page":post.page,"data-privacy":post.privacy,}).text(post.title))}
...
var raw=update(gps==null?{action:"post_create"}:{action:"post_create",location_lat:gps.latitude||"",location_lon:gps.longitude||"",place:gps.address||"",weather_id:gps.weather_id||""});switch(raw.Status){case 1:var r=raw.Values,post_id=r.post_id,subject=r.title;$("<li>",{title:"📝 "+subject,onclick:"if(this.dataset.active==null && !post_query(this.dataset.post_id)) $(this).remove();","data-active":"","data-post_id":post_id,"data-page":1,"data-privacy":0,}).text(subject).prependTo($(".catalog .diaries"));catalog_page(1);post_query(post_id);$(".alertify-dialog .alertify-button-ok").click();hj_mixpanel("Post Created",{post_id:post_id});ga_event_push("Create",{event_category:"Posts",event_label:"Create"});break;case 2:signin_required();break;default:hj_alert("糟糕，出錯了，請稍候再試");break}}
...
function post_created_date(d){var r=update({action:"post_update",field:"created_date",post_id:current_post()["post_id"],post_created:d});switch(r.Status){case 1:alertify.success("發文日期已更新："+d);hj_mixpanel("Created Date Updated",{post_id:post_id});ga_event_push("Created Date Updated",{event_category:"Posts",event_label:"Created Date Updated"});break;case 2:signin_required();break}}
...
function price_selector(pkg_id){var $p=$(".package[data-col='deposit']"),pkg={1:{unit:179,subtotal:179},3:{unit:149,subtotal:894},4:{unit:109,subtotal:1308}};if(pkg_id in pkg){pkg=pkg[pkg_id];$p.find(".price_unit").text(pkg.unit);$p.find("[data-subtotal]").attr({"data-subtotal":pkg.subtotal});$p.find(".btns_action[data-pkg]").attr({"data-pkg":pkg_id});$p.find(".price_selector li").removeAttr("data-active").filter("[data-pkg='"+pkg_id+"']").attr("data-active","")}
$p.find(".btns_action[data-pkg]").attr({"data-vat":user_country()=="TW"?1:0})}
function hj_picture(){var $f=$(".ajax-upload-dragdrop input[type='file']");if($f.length>0)
$f.attr({accept:"image/jpeg",onchange:"hj_uploader_preview(this.files);"}).get(0).click()}
function image_uploader_initialize(){var $e=$(".bk-page #editor_editable"),$u=$(".img_uploader");$u.find("#mulitplefileuploader").uploadFile({url:window.location.origin+"/update",dragDrop:!0,fileName:"myfile",allowedTypes:"jpg,jpeg",returnType:"json",showDone:!1,showDelete:!1,formData:{category:1},dynamicFormData:function(){return{post_id:current_post()["post_id"],privacy:current_post()["privacy"]}},onSubmit:function(files){saved(!1)},onSuccess:function(files,data,xhr){illustration_show(data.gallery_id);alertify.success("相片已上傳");post_revise();hj_mixpanel("Image Added",{post_id:current_post()["post_id"],privacy:current_post()["privacy"]});ga_event_push("Image Added",{event_category:"Posts",event_label:"Image Added"})},onError:function(files,status,errMsg,pd){msg()}})}
...
function hj_screenshot(){autosave_the_change();hj_loading(!0);var post_id=current_post()["post_id"],$pg=$(".bk-list .bk-page"),$e=$pg.find(".diary"),$d=$(".diary_screenshot");$pg.removeClass("scroll-smoothly").scrollTop(0).addClass("scroll-smoothly");$pg.find("#editor_editable").removeClass("editor_margin_bottom");$pg.find(".picture .far").hide();$e.addClass("screenshoted");hj_getScript("//cdn.jsdelivr.net/npm/html2canvas@1.0.0-alpha.12/dist/html2canvas.min.js",function(){window.html2canvas($e.get(0),{useCORS:!0,scale:window.devicePixelRatio,width:$e.get(0).scrollWidth+10,height:$e.get(0).scrollHeight}).then((c)=>{var b64=c.toDataURL("image/png"),subject=$pg.find("#subject").text();$d.empty();$("<a>",{target:"_blank",href:b64,download:"溫度日記_"+subject+".png",title:"下載："+subject}).html($("<img>",{src:b64})).appendTo($d).get(0).click();$e.removeClass("screenshoted").find("#editor_editable").addClass("editor_margin_bottom");$pg.find(".picture .far").show();popup_toggle(!0,"diary_screenshot");hj_loading(!1);hj_mixpanel("Post Screenshoted",{post_id:post_id});ga_event_push("Screenshot",{event_category:"Posts",event_label:"Screenshot"})})})}
...
return r.Status===1}
function signin_required(){alertify.set({labels:{ok:"立即登入",cancel:"否"}});alertify.confirm("<i class='fal fa-user-lock'></i> 登入以使用完整功能",function(e){if(e)window.location.href="https://hearty.me/?r="+window.location.href.replace(window.location.origin,"")+"#signin"});var r=update({action:"account_status"});if(!!r&&r.Status===1)window.location.reload(!0)}
