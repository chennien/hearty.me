...
var raw=update(gps==null?{action:"post_create"}:{action:"post_create",location_lat:gps.latitude||"",location_lon:gps.longitude||"",place:gps.address||"",weather_id:gps.weather_id||""});switch(raw.Status){case 1:var r=raw.Values,post_id=r.post_id,subject=r.title;$("<li>",{title:"📝 "+subject,onclick:"if(!post_query(this.dataset.post_id)) $(this).remove();","data-active":"","data-post_id":post_id,"data-page":1,"data-privacy":0,}).text(subject).prependTo($(".catalog .diaries"));catalog_page(1);post_query(post_id);hj_mixpanel("Post Created",{post_id:post_id});ga_event_push("Create",{event_category:"Posts",event_label:"Create"});break;case 2:signin_required();break;default:hj_alert("糟糕，出錯了，請稍候再試");break}}
